module Netgen
  class PluginHolod < Plugin
    ...
    def default_config
      {
        'bindir' => '/mnt/renato/git/rust/holo/target/debug',
        #'bindir' => '/mnt/renato/git/rust/holo/target/release',
    ...
    def node_start(node)
      ...
      #delay = holod_attr.dig('delay') || 20
      delay = holod_attr.dig('delay') || 2

      Netgen.log_info("starting holod", plugin: self, node: node)
      out = "#{@cfg['logdir']}/#{node.name}.out"
      err = "#{@cfg['logdir']}/#{node.name}.err"
      node.spawn("#{perf(node)} #{@cfg['bindir']}/holod", env: {"RUST_LOG" => "holo=trace", "RUST_BACKTRACE" => "1"}, options: { out: out, err: err })

      # Enter configuration.
      config_path = "#{node.mount_point}/#{@cfg['sysconfdir']}/holod.config"
      node.spawn("#{@cfg['bindir']}/holo-cli --file #{config_path}", delay: delay)
    end
